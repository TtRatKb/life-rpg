(() => {
  ...
  function startLevel(level,{replay=false}={}){const def=levelDef(level);if(!def)return;const allowed=level<=1||completedSet().has(level)||completedSet().has(level-1);if(!allowed)return;state().journey.active={id:`nonogram-l${level}${replay?"-replay":""}`,level,size:def.size,solution:def.solution,marks:Array(def.size*def.size).fill(0),replay:Boolean(replay),createdAt:Date.now(),updatedAt:Date.now(),completedAt:null,rewardEventId:null};mode="fill";persist("nonogram-start");openDialog();}
  function current(){return state().journey.active;}

  function render(){renderGrowth();renderDaily();renderProgress();renderLevels();renderTools();renderBoard();}
  ...
  function check(){const a=current();if(!a||a.completedAt)return;const correct=a.marks.every((mark,i)=>(mark===1)===(a.solution[i]==="1"));if(!correct){app.showToast?.("◩ Not quite yet — at least one filled cell is missing or misplaced.");els.board?.classList.add("shake-v314k");setTimeout(()=>els.board?.classList.remove("shake-v314k"),350);return;}complete(a);}
  function complete(a){const already=completedSet().has(a.level);a.completedAt=Date.now();a.updatedAt=a.completedAt;if(a.replay||already){state().completed.push({id:a.id,level:a.level,replay:true,completedAt:a.completedAt,rewardEventId:null});persist("nonogram-replay");app.showToast?.(`↻ Nonogram Level ${a.level} replay solved · no duplicate rewards`);return;}const reward=award(a);state().journey.completedLevels.push(a.level);state().journey.completedLevels=[...new Set(state().journey.completedLevels)].sort((x,y)=>x-y);state().stats.solved=Number(state().stats.solved||0)+1;state().completed.push({id:a.id,level:a.level,replay:false,completedAt:a.completedAt,rewardEventId:reward.eventId||null,reward:{xp:reward.xp,realmXP:reward.realmXP,statXP:reward.statXP,coins:reward.coins,storyEnergy:reward.storyEnergy}});state().completed=state().completed.slice(-300);persist("nonogram-complete");const next=nextLevel();app.showToast?.(`◩ Level ${a.level} complete · +${reward.xp} XP · +${app.formatEnergy?.(reward.storyEnergy)??reward.storyEnergy} 🔥 · +${reward.coins} 🪙${next?` · Level ${next} unlocked`:" · Chapter 1 complete!"}`);}
