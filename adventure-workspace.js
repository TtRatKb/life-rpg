(() => {
  ...
  function renderDraft() {
    ...
    if (els.images) {
      els.images.innerHTML = draft.images.length ? draft.images.map(image => `<figure class="workspace-image-chip-v310"><img src="${escAttr(image.data)}" alt="" /><button type="button" data-step-image-remove="${escAttr(image.id)}">×</button></figure>`).join("") : `<small class="workspace-empty-inline-v310">No screenshots/images added.</small>`;
    }
    renderRewardMeter();
  }
  ...
  function renderWorkspace(item) {
    ...
    if (els.workspaceImages) els.workspaceImages.innerHTML = w.images.length ? w.images.map(image => `<figure class="workspace-gallery-image-v310"><img src="${escAttr(image.data)}" alt="${escAttr(image.name || "Project reference")}" /><figcaption>${esc(image.name || "Reference image")}</figcaption></figure>`).join("") : `<div class="workspace-empty-v310">No images saved yet.</div>`;
