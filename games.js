(() => {
  ...
  const els = {
    ...
    steamPlaytimeHours: byId("gameSteamPlaytimeHours"),
    steamGameSpoilerMode: byId("gameSteamSpoilerMode"),
  ...
  function ensureState() {
    ...
    model.items.forEach(game => {
      ...
      if (!["inherit", "strict", "steam-hidden", "off"].includes(game.steamSpoilerMode)) { game.steamSpoilerMode = "inherit"; changed = true; }
      ...
      if (!game.steamPlaytimeSync || typeof game.steamPlaytimeSync !== "object" || Array.isArray(game.steamPlaytimeSync)) {
        game.steamPlaytimeSync = defaultSteamPlaytimeSyncState(game);
        changed = true;
      } else {
        const playtimeSync = game.steamPlaytimeSync;
        if (Number(playtimeSync.schemaVersion || 0) < STEAM_PLAYTIME_SYNC_SCHEMA) { playtimeSync.schemaVersion = STEAM_PLAYTIME_SYNC_SCHEMA; changed = true; }
        if (!Number.isFinite(Number(playtimeSync.baselineMinutes))) { playtimeSync.baselineMinutes = Math.max(0, Number(game.steamPlaytimeMinutes || 0)); changed = true; }
        ...
        if (!Number.isFinite(Number(playtimeSync.totalImportedMinutes))) { playtimeSync.totalImportedMinutes = 0; changed = true; }
        if (!Number.isFinite(Number(playtimeSync.totalCoveredMinutes))) { playtimeSync.totalCoveredMinutes = 0; changed = true; }
      }
  ...
  function steamRarity(globalPercent) {
    if (globalPercent === null || globalPercent === undefined || globalPercent === "") {
      return { label: "Unknown rarity", multiplier: 1 };
    }
    const percent = Number(globalPercent);
    if (!Number.isFinite(percent) || percent < 0 || percent > 100) {
      return { label: "Unknown rarity", multiplier: 1 };
    }
    if (percent < 1) return { label: "Legendary", multiplier: 2.75 };
    if (percent < 5) return { label: "Ultra rare", multiplier: 2.1 };
    if (percent < 10) return { label: "Very rare", multiplier: 1.65 };
    if (percent < 25) return { label: "Rare", multiplier: 1.35 };
    if (percent < 50) return { label: "Uncommon", multiplier: 1.15 };
    return { label: "Common", multiplier: 1 };
  }

  function steamSpoilerModeForGame(game = null, formOverride = "") {
    const override = String(formOverride || game?.steamSpoilerMode || "inherit");
    if (["strict", "steam-hidden", "off"].includes(override)) return override;
    return steamSettings().spoilerMode || "strict";
  }

  function steamSpoilerProtected(item = {}, game = null, formOverride = "") {
    ...
    const mode = steamSpoilerModeForGame(game, formOverride);
    if (mode === "strict") return true;
    if (mode === "steam-hidden") return Boolean(item.hidden || item.steamHidden);
    return false;
  }

  function steamSafeName(item = {}, game = null, formOverride = "") {
    return steamSpoilerProtected(item, game, formOverride) ? "Locked Steam achievement" : String(item.name || item.text || item.apiName || "Steam achievement");
  }

  function steamSafeDescription(item = {}, game = null, formOverride = "") {
    if (steamSpoilerProtected(item, game, formOverride)) return "Spoiler Shield is hiding this achievement until you unlock it.";
  ...
  function renderBoard() {
    ...
    const games = model().items.filter(game => {
      ...
      if (query) {
        const haystack = [game.title, game.platform, game.notes, game.description, game.developer, game.publisher, ...(game.genres || []), ...game.goals.map(goal => goal.source === "steam" && steamSpoilerProtected(goal, game) ? "" : goal.text)].join(" ").toLowerCase();
  ...
  function parseSteamHoursInput(value) {
  ...
  function gameCardMarkup(game) {
    ...
    const localLastPlayed = Number(game.lastPlayedAt || 0);
    const steamLastPlayed = Number(game.steamLastPlayedAt || 0);
    const lastPlayed = effectiveLastPlayed ? `Smart shelf · last played ${humanAgo(effectiveLastPlayed)}` : "Smart shelf · not played yet";
    const lifePlaytime = game.totalMinutes > 0
      ? `Life RPG · ${formatDuration(game.totalMinutes)} logged${localLastPlayed ? ` · last logged ${humanAgo(localLastPlayed)}` : ""}`
      : "Life RPG · no session logged yet";
    const steamPlaytime = Number(game.steamPlaytimeMinutes || 0) > 0
      ? `Steam · ${formatSteamHours(game.steamPlaytimeMinutes)} total${game.steamPlaytimeManual ? " · manual" : ""}${steamLastPlayed ? ` · played ${humanAgo(steamLastPlayed)}` : ""}`
      : steamLastPlayed ? `Steam · played ${humanAgo(steamLastPlayed)}` : "";
  ...
  function goalMarkup(game, goal) {
    ...
    const protectedSteam = goal.source === "steam" && steamSpoilerProtected(goal, game);
    const source = goal.source === "steam"
      ? `<small class="game-goal-source-v312">Steam${steamState}${protectedSteam ? " · Spoiler protected" : goal.globalPercent != null ? ` · ${esc(rarity.label)} · ${formatNumber(goal.globalPercent)}%` : ""}</small>`
      : "";
    const displayText = goal.source === "steam" ? steamSafeName({ ...goal, name: goal.text }, game) : goal.text;
    const safeDescription = goal.source === "steam" ? steamSafeDescription(goal, game) : "";
  ...
  function openGameDialog(id = "") {
    ...
    if (els.steamPlaytimeHours) els.steamPlaytimeHours.value = Number(game?.steamPlaytimeMinutes || 0) > 0 ? (Number(game.steamPlaytimeMinutes) / 60).toFixed(1) : "";
    if (els.steamGameSpoilerMode) els.steamGameSpoilerMode.value = ["strict", "steam-hidden", "off"].includes(game?.steamSpoilerMode) ? game.steamSpoilerMode : "inherit";
  ...
  function saveGame(event) {
    ...
    const now = Date.now();
    const manualSteamHours = parseSteamHoursInput(els.steamPlaytimeHours?.value);
    if (Number.isNaN(manualSteamHours)) {
      showToast("Check Steam playtime", "Enter Steam hours as a decimal number, for example 25.4.");
      els.steamPlaytimeHours?.focus();
      return;
    }
    ...
    const game = {
      ...
      steamSpoilerMode: ["strict", "steam-hidden", "off"].includes(els.steamGameSpoilerMode?.value) ? els.steamGameSpoilerMode.value : "inherit",
    ...
    let manualPlaytimeResult = null;
    if (manualSteamHours !== null) {
      const manualMinutes = Math.max(0, Math.round(manualSteamHours * 60));
      const priorMinutes = Math.max(0, Math.round(Number(existing?.steamPlaytimeMinutes || 0)));
      const priorDisplayedHours = Number((priorMinutes / 60).toFixed(1));
      const manualValueChanged = !existing || Math.abs(manualSteamHours - priorDisplayedHours) >= 0.05;
      if (manualValueChanged) {
        manualPlaytimeResult = processSteamPlaytimeSync(game, manualMinutes, now, Number(game.steamLastPlayedAt || 0));
        game.steamPlaytimeMinutes = manualMinutes;
        game.steamLibrarySyncedAt = now;
        game.steamPlaytimeManual = true;
        steamPlaytimeSyncState(game).lastError = "";
      }
    }
    ...
    const manualPlaytimeText = manualPlaytimeResult
      ? manualPlaytimeResult.baselined
        ? `Steam baseline set to ${formatSteamHours(game.steamPlaytimeMinutes)} · no retro rewards`
        : manualPlaytimeResult.importedMinutes > 0
          ? `Steam total ${formatSteamHours(game.steamPlaytimeMinutes)} · ${formatSteamHours(manualPlaytimeResult.importedMinutes)} new playtime logged`
          : `Steam total updated to ${formatSteamHours(game.steamPlaytimeMinutes)}`
      : "";
  ...
  function resetGameDialogForAnother(defaults = {}) {
    ...
    if (els.steamAppIdInput) els.steamAppIdInput.value = "";
    if (els.steamPlaytimeHours) els.steamPlaytimeHours.value = "";
    if (els.steamGameSpoilerMode) els.steamGameSpoilerMode.value = "inherit";
  ...
  function renderGoalOptions(game) {
    ...
    els.logGoal.innerHTML = `<option value="">No specific goal</option>${open.map(goal => {
      const label = goal.source === "steam" ? steamSafeName({ ...goal, name: goal.text }, game) : goal.text;
  ...
  function renderLogPreview() {
    ...
    const goalLabel = goal ? (goal.source === "steam" ? steamSafeName({ ...goal, name: goal.text }, game) : goal.text) : "";
  ...
  async function syncAllSteamGamesManual() {
    ...
    const detail = [
      ...
      libraryResult?.baselined ? `${libraryResult.baselined} playtime baseline${libraryResult.baselined === 1 ? "" : "s"} created · no retro rewards` : "",
      libraryResult?.importedMinutes ? `${formatDuration(libraryResult.importedMinutes)} new Steam playtime auto-logged` : "",
      libraryResult?.coveredMinutes ? `${formatDuration(libraryResult.coveredMinutes)} already covered by local game logs` : "",
      libraryResult?.skillXp ? `+${formatNumber(libraryResult.skillXp)} Skill XP from new playtime` : "",
      libraryResult?.realmXp ? `+${formatNumber(libraryResult.realmXp)} Realm XP from new playtime` : "",
  ...
  function renderSteamSection() {
    ...
    if (els.steamGameSpoilerMode && document.activeElement !== els.steamGameSpoilerMode) {
      const game = findGame(els.editId?.value);
      els.steamGameSpoilerMode.value = ["strict", "steam-hidden", "off"].includes(game?.steamSpoilerMode) ? game.steamSpoilerMode : (els.steamGameSpoilerMode.value || "inherit");
    }
  ...
  function defaultSteamPlaytimeSyncState(game = {}) {
    const existingMinutes = Math.max(0, Number(game?.steamPlaytimeMinutes || 0));
    const existingSyncAt = Math.max(0, Number(game?.steamLibrarySyncedAt || 0));
    return {
      schemaVersion: STEAM_PLAYTIME_SYNC_SCHEMA,
      baselineMinutes: existingMinutes,
      ...
      totalImportedMinutes: 0,
      totalCoveredMinutes: 0
    };
  }

  function steamPlaytimeSyncState(game = {}) {
    if (!game.steamPlaytimeSync || typeof game.steamPlaytimeSync !== "object" || Array.isArray(game.steamPlaytimeSync)) {
      game.steamPlaytimeSync = defaultSteamPlaytimeSyncState(game);
    }
    const sync = game.steamPlaytimeSync;
    sync.schemaVersion = STEAM_PLAYTIME_SYNC_SCHEMA;
    if (!Number.isFinite(Number(sync.baselineMinutes))) sync.baselineMinutes = Math.max(0, Number(game.steamPlaytimeMinutes || 0));
    ...
    if (!Number.isFinite(Number(sync.totalImportedMinutes))) sync.totalImportedMinutes = 0;
    if (!Number.isFinite(Number(sync.totalCoveredMinutes))) sync.totalCoveredMinutes = 0;
    return sync;
  }

  function steamPlaytimeSkillId(game = {}) {
    return game.role === "japanese" ? "language-learning" : "recreation-play";
  }

  function steamPlaytimeRealm(game = {}) {
    return (ROLES[game.role] || ROLES.fun).realm;
  }

  function steamTimeSkillXp(minutes) {
    const safe = Math.max(0, Number(minutes || 0));
    return safe > 0 ? Math.round((2 * Math.sqrt(safe) + Number.EPSILON) * 100) / 100 : 0;
  }

  function manualGameLogsCoveringSteamInterval(game, fromAt, toAt) {
    return model().logs.filter(log => {
      if (!log || log.gameId !== game.id || log.steamImported || log.source === "steam-playtime") return false;
      const at = Number(log.at || log.createdAt || 0);
      if (!Number.isFinite(at) || at <= fromAt || at > toAt) return false;
      return Math.max(0, Number(log.minutes || 0)) > 0;
    });
  }

  function matchingGamingTimeEntries(game, fromAt, toAt) {
    const title = normalizeText(game?.title || "");
    if (!title) return [];
    return (app.getState().timeTracking?.entries || []).filter(entry => {
      if (!entry || entry.categoryId !== "gaming") return false;
      const at = new Date(entry.endAt || entry.startAt || entry.createdAt || 0).getTime();
      if (!Number.isFinite(at) || at <= fromAt || at > toAt) return false;
      const label = normalizeText(entry.label || "");
      return Boolean(label && (label.includes(title) || title.includes(label)));
    });
  }

  function representedSteamInterval(game, fromAt, toAt) {
    const gameLogs = manualGameLogsCoveringSteamInterval(game, fromAt, toAt);
    const timeEntries = matchingGamingTimeEntries(game, fromAt, toAt);

    const gameMinutes = gameLogs.reduce((sum, log) => sum + Math.max(0, Number(log.minutes || 0)), 0);
    const timeMinutes = timeEntries.reduce((sum, entry) => {
      const seconds = Math.max(0, Number(entry.durationSeconds || 0));
      return sum + (seconds > 0 ? seconds / 60 : Math.max(0, Number(entry.minutes || 0)));
    }, 0);

    const gameSkillXp = gameLogs.reduce((sum, log) => {
      const override = Number(log.skillXpOverride);
      if (Number.isFinite(override) && override >= 0) return sum + override;
      return sum + steamTimeSkillXp(log.minutes);
    }, 0);

    const timeSkillXp = timeEntries.reduce((sum, entry) => {
      const seconds = Math.max(0, Number(entry.durationSeconds || 0));
      const minutes = seconds > 0 ? seconds / 60 : Math.max(0, Number(entry.minutes || 0));
      return sum + steamTimeSkillXp(minutes);
    }, 0);

    const gameRealmXp = gameLogs.reduce((sum, log) => sum + Math.max(0, Number(log.realmXP || 0)), 0);

    return {
      gameLogs,
      timeEntries,
      minutes: gameMinutes + timeMinutes,
      skillXp: gameSkillXp + timeSkillXp,
      realmXp: gameRealmXp
    };
  }

  function steamPlaytimeRewardKey(game, previousMinutes, remoteMinutes) {
    return `${String(game?.steamAppId || "").trim()}:playtime:${Math.max(0, Math.round(previousMinutes))}-${Math.max(0, Math.round(remoteMinutes))}`;
  }

  function processSteamPlaytimeSync(game, remoteMinutes, syncedAt = Date.now(), remoteLastPlayedAt = 0) {
    if (!game?.steamAppId) return { gameId: game?.id || "", matched: false, baselined: false, importedMinutes: 0, coveredMinutes: 0, remoteDeltaMinutes: 0, skillXp: 0, realmXp: 0 };

    const sync = steamPlaytimeSyncState(game);
    const remote = Math.max(0, Math.round(Number(remoteMinutes || 0)));
    const priorBaseline = Math.max(0, Math.round(Number(sync.baselineMinutes || 0)));
    ...
    // First known Steam total is a reward-free baseline, exactly like old achievements.
    if (!Number(sync.verifiedAt || 0)) {
      ...
      return {
        ...
        importedMinutes: 0,
        coveredMinutes: 0,
        remoteDeltaMinutes: 0,
        skillXp: 0,
        realmXp: 0
      };
    }

    // Never lower a baseline. A temporary API correction must not become fake future playtime.
    if (remote <= priorBaseline) {
      sync.baselineMinutes = priorBaseline;
      sync.baselineAt = syncedAt;
      return {
        gameId: game.id,
        matched: true,
    ...
    const remoteDelta = remote - priorBaseline;
    const represented = representedSteamInterval(game, priorAt, syncedAt);
    const coveredMinutes = Math.min(remoteDelta, Math.max(0, Math.round(represented.minutes)));
    const importedMinutes = Math.max(0, remoteDelta - coveredMinutes);

    const targetSkillXp = steamTimeSkillXp(remoteDelta);
    const importedSkillXp = importedMinutes > 0
      ? Math.max(0, Math.round((targetSkillXp - Math.min(targetSkillXp, represented.skillXp) + Number.EPSILON) * 100) / 100)
      : 0;

    // Steam auto-playtime intentionally supplies Realm XP + Skill XP, but not Coins,
    // Story Energy or an achievement-style Character reward. Achievements remain the
    // milestone economy; playtime remains practice.
    const targetRealmXp = remoteDelta > 0 ? Math.max(1, Math.round(targetSkillXp)) : 0;
    const importedRealmXp = importedMinutes > 0
      ? Math.max(0, targetRealmXp - Math.min(targetRealmXp, Math.round(represented.realmXp)))
      : 0;

    let reward = null;
    const rewardKey = steamPlaytimeRewardKey(game, priorBaseline, remote);
    const alreadyRewarded = (app.getState().rewardLedger?.events || []).some(event =>
      event?.source === "steam-playtime" && event?.sourceId === rewardKey
    );

    if (importedRealmXp > 0 && !alreadyRewarded) {
      reward = app.awardActivity?.({
        source: "steam-playtime",
        sourceId: rewardKey,
        label: `${game.title} · Steam playtime`,
        realm: steamPlaytimeRealm(game),
        capability: null,
        xp: 0,
        realmXP: importedRealmXp,
        statXP: 0,
        coins: 0,
        storyEnergyBase: 0,
        progressionRelevant: true,
        at: new Date(syncedAt).toISOString(),
        metadata: {
          steamPlaytime: true,
          steamAppId: String(game.steamAppId || ""),
          remoteDeltaMinutes: remoteDelta,
          importedMinutes,
          manualCoveredMinutes: coveredMinutes,
          skillXP: importedSkillXp,
          skillId: steamPlaytimeSkillId(game),
          intervalStartAt: priorAt || null,
          intervalEndAt: syncedAt,
          steamLastPlayedAt: remoteLastPlayedAt || 0,
          dateUncertain: true
        }
      }) || null;
    }

    if (importedMinutes > 0) {
      model().logs.push({
        id: makeId("glog-steam"),
        gameId: game.id,
        at: syncedAt,
        date: todayKey(),
        minutes: importedMinutes,
        amount: importedMinutes,
        ...
        source: "steam-playtime",
        steamImported: true,
        steamIntervalStartAt: priorAt || null,
        steamIntervalEndAt: syncedAt,
        steamRemoteDeltaMinutes: remoteDelta,
        steamManualCoveredMinutes: coveredMinutes,
        steamLastPlayedAt: remoteLastPlayedAt || 0,
        skillXpOverride: importedSkillXp,
        skillId: steamPlaytimeSkillId(game),
        dateUncertain: true,
        xp: 0,
        realmXP: Number(reward?.realmXP || importedRealmXp || 0),
        statXP: 0,
        storyEnergy: 0,
        rawStoryEnergy: 0,
      ...
      if (model().logs.length > MAX_LOGS) model().logs = model().logs.slice(-MAX_LOGS);
      game.totalMinutes = Number(game.totalMinutes || 0) + importedMinutes;
    }

    sync.baselineMinutes = remote;
    sync.baselineAt = syncedAt;
    sync.lastRemoteDeltaMinutes = remoteDelta;
    sync.lastImportedMinutes = importedMinutes;
    sync.lastCoveredMinutes = coveredMinutes;
    sync.totalImportedMinutes = Number(sync.totalImportedMinutes || 0) + importedMinutes;
    sync.totalCoveredMinutes = Number(sync.totalCoveredMinutes || 0) + coveredMinutes;
  ...
  function applySteamLibrary(data = {}, syncedAt = Date.now(), onlyAppId = "") {
    ...
    for (const game of model().items) {
      ...
      result.matched += 1;
      result.baselined += playtime.baselined ? 1 : 0;
      result.importedMinutes += Number(playtime.importedMinutes || 0);
      result.coveredMinutes += Number(playtime.coveredMinutes || 0);
      result.remoteDeltaMinutes += Number(playtime.remoteDeltaMinutes || 0);
      result.skillXp += Number(playtime.skillXp || 0);
      result.realmXp += Number(playtime.realmXp || 0);
      result.games.push(playtime);

      game.steamPlaytimeMinutes = Math.max(Number(game.steamPlaytimeMinutes || 0), remoteForever);
      game.steamPlaytimeManual = false;
  ...
  async function syncSteamLibrary({ appId = "", silent = true } = {}) {
    ...
    try {
      ...
      if (!silent) {
        ...
        if (result.skillXp) bits.push(`+${formatNumber(result.skillXp)} Skill XP`);
        if (result.realmXp) bits.push(`+${formatNumber(result.realmXp)} Realm XP`);
  ...
  async function loadSteamAchievements() {
    ...
    try {
      ...
      const formSpoilerMode = els.steamGameSpoilerMode?.value || existingGame?.steamSpoilerMode || "inherit";
      pendingSteamAchievements = items.map(item => {
        const record = syncRecords[item.apiName] || null;
        const protectedLocked = steamSpoilerProtected(item, existingGame, formSpoilerMode);
        return {
          ...item,
          selected: !item.achieved && !protectedLocked && !item.hidden && item.group === "recommended" && !existingSteam.has(item.apiName),
          queued: false,
  ...
  function steamUnlockDayKey(unlockTime = 0) {
    const raw = Math.max(0, Number(unlockTime || 0));
    const ms = raw > 10_000_000_000 ? raw : raw > 0 ? raw * 1000 : Date.now();
    return dateKeyFromValue(ms);
  }

  function steamUnlockRewardCountForDay(dayKey) {
    if (!dayKey) return 0;
    return (app.getState().rewardLedger?.events || []).filter(event => {
      if (event?.source !== "steam-achievement" || event?.duplicate) return false;
      const unlockTime = Number(event?.metadata?.steamUnlockTime || 0);
      if (!(unlockTime > 0)) return false;
      return steamUnlockDayKey(unlockTime) === dayKey;
    }).length;
  }
  ...
  function awardSteamUnlock(game, remote, goal, batchIndex) {
    ...
    const reward = app.awardActivity?.({
      ...
      storyEnergyBase: Number(Math.min(base.storyEnergyBase + 2, base.storyEnergyBase * multiplier).toFixed(2)),
      progressionRelevant: true,
      metadata: {
        ...
        steamRewardMultiplier: Number(multiplier.toFixed(3)),
        steamRaritySnapshotAt: new Date().toISOString()
      }
    }) || null;
    return reward;
  }
  ...
  function steamPlaytimeNoteForGame(playtimeResult, game) {
    ...
    if (row.importedMinutes) {
      const covered = row.coveredMinutes ? ` · ${formatDuration(row.coveredMinutes)} already covered locally` : "";
      const skill = row.skillXp ? ` · +${formatNumber(row.skillXp)} Skill XP` : "";
      const realm = row.realmXp ? ` · +${formatNumber(row.realmXp)} ${steamPlaytimeRealm(game)} XP` : "";
      return ` · ${formatDuration(row.importedMinutes)} new Steam playtime logged${skill}${realm}${covered}`;
    }
    if (row.coveredMinutes) return ` · ${formatDuration(row.coveredMinutes)} Steam delta already covered by local logs`;
    return "";
  }

  function processSteamAchievementSync(game, steamItems, data, syncedAt = Date.now(), { silent = true, source = "sync", playtimeResult = null } = {}) {
    ...
    let changed = false;
    const newUnlocks = [];
    const rewardBatchCursor = new Map();
    let historicalImported = 0;
    let matchedGoals = 0;
    ...
    for (const remote of steamItems) {
      ...
      if (achievementIsNewlyUnlocked) {
        ...
        if (unseenButOld) {
          ...
          const unlockDay = steamUnlockDayKey(remote.unlockTime);
          const batchIndex = steamUnlockRewardCountForDay(unlockDay) + Number(rewardBatchCursor.get(unlockDay) || 0);
          ...
          if (reward) {
            ...
            rewardBatchCursor.set(unlockDay, Number(rewardBatchCursor.get(unlockDay) || 0) + 1);
          }
    ...
    if (!silent) {
      const playtimeNote = steamPlaytimeNoteForGame(playtimeResult, game);
      if (!steamItems.length) {
        showToast("Steam returned no achievements", `${game.title} · connection works, but this game returned 0 achievements. Check the App ID and Steam privacy/API availability.${playtimeNote}`);
      } else if (isBaseline) {
        showToast("Steam baseline saved ✓", `${game.title} · ${sync.unlocked}/${sync.total} already unlocked · ${historicalImported} added to Life RPG as Historical · no retro rewards${playtimeNote}.`);
        ...
        showToast(`${newUnlocks.length} new Steam achievement${newUnlocks.length === 1 ? "" : "s"} 🏆`, `${game.title} · +${totals.xp} XP · +${app.formatEnergy?.(totals.storyEnergy) ?? totals.storyEnergy} 🔥 · +${totals.coins} 🪙${repaired}${playtimeNote}`);
      } else if (historicalImported) {
        showToast("Steam history reconciled ✓", `${game.title} · ${sync.unlocked}/${sync.total} unlocked on Steam · ${historicalImported} historical achievement${historicalImported === 1 ? "" : "s"} added · 0 retro rewards${playtimeNote}.`);
      } else {
        showToast("Steam is up to date ✓", `${game.title} · ${sync.unlocked}/${sync.total} achievements unlocked · ${matchedGoals} already represented in Life RPG${playtimeNote}.`);
      }
    }
  ...
  function steamSyncMarkup(game) {
    ...
    const manualSteamPlaytime = Boolean(game.steamPlaytimeManual);
    const playtimeAuto = manualSteamPlaytime
      ? playtimeSync.verifiedAt
        ? "Manual Steam total · enter the newer Steam number in Edit to log the difference"
        : "Manual Steam total"
      : playtimeSync.lastError
        ...
        : playtimeSync.verifiedAt
          ...
          : configured
            ...
            : "";
    const steamContext = Number(game.steamPlaytimeMinutes || 0) > 0 ? `Steam ${formatSteamHours(game.steamPlaytimeMinutes)} total${manualSteamPlaytime ? " · manual" : ""}${Number(game.steamPlaytime2WeeksMinutes || 0) > 0 && !manualSteamPlaytime ? ` · ${formatDuration(game.steamPlaytime2WeeksMinutes)} in last 2 weeks` : ""}${game.steamLastPlayedAt ? ` · played ${humanAgo(game.steamLastPlayedAt)}` : ""}${playtimeAuto ? ` · ${playtimeAuto}` : ""}` : playtimeAuto;
  ...
  function renderSteamAchievements() {
    ...
    const game = findGame(els.editId?.value);
    const formSpoilerMode = els.steamGameSpoilerMode?.value || game?.steamSpoilerMode || "inherit";
    const mode = steamSpoilerModeForGame(game, formSpoilerMode);
    const strict = mode === "strict";
    els.steamSelectAll?.classList.toggle("hidden", !has || strict);
    els.steamSelectRecommended?.classList.toggle("hidden", !has || strict);
    els.steamClearSelection?.classList.toggle("hidden", !has || strict);
    els.steamHiddenLabel?.classList.toggle("hidden", !has || strict || mode !== "steam-hidden" || !pendingSteamAchievements.some(item => item.hidden && !item.achieved));
    const includeHidden = Boolean(els.steamIncludeHidden?.checked);
    const lockedProtected = pendingSteamAchievements.filter(item => !item.achieved && steamSpoilerProtected(item, game, formSpoilerMode));
    const visible = pendingSteamAchievements.filter(item => {
      if (strict && !item.achieved) return false;
      if (mode === "steam-hidden" && item.hidden && !item.achieved && !includeHidden) return false;
      return true;
    });
    const protectedSummary = strict && lockedProtected.length
      ? `<div class="game-steam-spoiler-summary-v314x"><span>🔒</span><div><strong>${lockedProtected.length} locked achievement${lockedProtected.length === 1 ? "" : "s"} protected</strong><small>Titles, descriptions, icons, rarity and ordering stay hidden. They still sync and reward automatically when Steam reports the unlock.</small></div></div>`
      : "";
    const rows = visible.map(item => {
      ...
      const displayName = steamSafeName(item, game, formSpoilerMode);
      const description = steamSafeDescription(item, game, formSpoilerMode) || (item.hidden ? "Hidden Steam achievement" : "No description supplied by Steam.");
      return `<button type="button" class="game-steam-achievement-v312 ${chosen ? "selected" : ""} ${item.achieved ? "achieved" : ""} ${item.alreadyImported ? "imported" : ""} ${steamSpoilerProtected(item, game, formSpoilerMode) ? "spoiler-protected" : ""}" data-game-steam-achievement="${actualIndex}" aria-pressed="${chosen ? "true" : "false"}" ${item.alreadyImported ? "disabled" : ""}>
    ...
    els.steamAchievementList.innerHTML = `${protectedSummary}${rows}` || `<p class="muted">No achievements to show with the current filter.</p>`;
  ...
  function updateSteamAddButton() {
    ...
    const game = findGame(els.editId?.value);
    const formSpoilerMode = els.steamGameSpoilerMode?.value || game?.steamSpoilerMode || "inherit";
    const strict = steamSpoilerModeForGame(game, formSpoilerMode) === "strict";
    els.steamAddSelected?.classList.toggle("hidden", !pendingSteamAchievements.length || strict);
  ...
  function exposeApi() {
    window.LifeRPGGames = {
      ...
      amountLabel,
      playButtonLabel,
      safeGoalLabel: (goal, game = null) => goal?.source === "steam" ? steamSafeName({ ...goal, name: goal.text }, game) : String(goal?.text || ""),
